...
- 2026-05-09 — Initial "LIVE" claim was premature. Hatchery v65 + Marketplace v5 went live with **empty** `TelemetryConfig.luau` because the publish scripts didn't auto-source `.env` and the calling shell hadn't sourced it. Build silently wrote `endpoint = ""` stub. Roblox-side `TelemetryBoot` detected empty values and fell through to "buffer + drop" mode, so zero events from real playthroughs landed in KV.
- 2026-05-09 — Auto-source `.env` added to all 3 publish scripts (commit `608e9a1`). Republished Hatchery v67, Marketplace v7, Corridors v6 with `grep -c bab-telemetry *.rbxlx` returning 1 each — endpoint URL now baked into the live rbxlx.
- 2026-05-09 — Also fixed `Enum.Font.GothamItalic` (invalid value, halted MarketplaceUI client-side, blocked the sell modal from popping). Now `Enum.Font.SourceSansItalic` (commit `b560e76`).
- **STATUS: Phase 0 FULLY VERIFIED end-to-end** (2026-05-09 17:16 PT). Live playthrough produced: 3 `session_start`, 1 `session_end`, 7 `egg_purchased`, 7 `pod_ripened`, 6 `harvest_decision`, 1 `sell_payout`, 1 `smoke_test`. The `sell_payout` is the proof — full asset/liability pricing flow is telemetered. ADR-3 (open P0 since 4 months ago) closed.
